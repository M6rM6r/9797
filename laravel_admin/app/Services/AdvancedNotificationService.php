...
class AdvancedNotificationService
{
    ...
    protected function initializeRules(): void
    {
        $this->notificationRules = [
            'rate_limiting' => [
                'max_per_hour' => 10,
                'max_per_day' => 50,
                'cooldown_between_same_type' => 300 // 5 minutes
            ],
            'quiet_hours' => [
                'enabled' => true,
                'start' => '22:00',
    ...
    {
        try {
            $scheduledNotifications = Notification::where('scheduled_at', '<=', now())
                ->where('status', 'scheduled')
                ->get();
            
            $results = [];
            foreach ($scheduledNotifications as $notification) {
                $result = $this->processScheduledNotification($notification);
                $results[] = $result;
            }
            ...
            Log::error('Error sending scheduled notifications', [
                'error' => $e->getMessage()
            ]);
     ...
     */
    protected function processScheduledNotification(Notification $notification): array
    {
        try {
            ...
            $results = [];
            foreach ($channels as $channel) {
                $result = $this->queueNotification($notification, $channel, $data);
                $results[$channel] = $result;
            }
    ...
    protected function filterContent(array $content): array
    {
        ...
        // Filter HTML tags (simplified)
        if (!empty($rules['allowed_html_tags'])) {
            // This would use a proper HTML purifier in production
    ...
    {
        $profanity = ['كلمة سيئة', 'bad word']; // Add actual profanity words
    ...
    protected function queueNotification(Notification $notification, string $channel, array $data): array
    {
        try {
            ...
            switch ($channel) {
                ...
                case 'webhook':
                    $this->sendWebhookNotification($notification, $data);
                    break;
            }
    ...
    protected function storeInAppNotification(Notification $notification, array $data): void
    {
        ...
        // This method can be extended to add additional in-app specific logic
        Cache::put("in_app_notification_{$notification->id}", $data, 86400); // 24 hours
    }
    
    /**
     * Send webhook notification
     */
    protected function sendWebhookNotification(Notification $notification, array $data): void
    {
        $webhookUrl = config('services.notifications.webhook_url');
    ...
    protected function updateRateLimitingCache(int $userId, string $type): void
    {
        ...
        Cache::put($rateLimitKey, $currentData, 3600); // 1 hour
    }
    
    /**
     * Update frequency capping cache
     */
    protected function updateFrequencyCappingCache(int $userId, string $type, array $data): void
    {
        $capKey = "notification_freq_cap_{$userId}";
        $currentData = Cache::get($capKey, []);
        ...
        // Update counters
        if (isset($data['coupon_id'])) {
            $currentData['coupon'][$data['coupon_id']][$today] = 
                ($currentData['coupon'][$data['coupon_id']][$today] ?? 0) + 1;
        }
        
        if (isset($data['store_id'])) {
            $currentData['store'][$data['store_id']][$today] = 
                ($currentData['store'][$data['store_id']][$today] ?? 0) + 1;
        }
        
        $currentData['type'][$type][$today] = 
            ($currentData['type'][$type][$today] ?? 0) + 1;
        
        Cache::put($capKey, $currentData, 86400); // 24 hours
    }
